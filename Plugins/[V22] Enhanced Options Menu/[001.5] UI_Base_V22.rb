#===============================================================================
# V22 UI Base Classes - Backported to v21
# Provides the UI module, BaseVisuals, BaseScreen, and BitmapSprite extensions
# that are required by the V22 Options Menu.
#===============================================================================

#===============================================================================
# Input compatibility stubs for v22 control remapping features.
# These must be defined BEFORE UI_Options_V22.rb loads.
#===============================================================================
module Input
  # Flag: true only if v22's actual key remapping system exists
  V22_REMAPPING_AVAILABLE = const_defined?(:REMAP_KEYBOARD_KEYS)

  # Stub constants (no-ops on v21, real values on v22)
  QUICK_UP   = :JUMPUP   if !const_defined?(:QUICK_UP)
  QUICK_DOWN = :JUMPDOWN if !const_defined?(:QUICK_DOWN)
  REMAP_KEYBOARD_KEYS           = {} if !const_defined?(:REMAP_KEYBOARD_KEYS)
  REMAP_GAMEPAD_BUTTONS          = {} if !const_defined?(:REMAP_GAMEPAD_BUTTONS)
  REMAP_GAMEPAD_AXIS             = {} if !const_defined?(:REMAP_GAMEPAD_AXIS)
  DEFAULT_INPUT_MAPPINGS         = {} if !const_defined?(:DEFAULT_INPUT_MAPPINGS)
  DEFAULT_INPUT_MAPPINGS_REMAPPABLE = {} if !const_defined?(:DEFAULT_INPUT_MAPPINGS_REMAPPABLE)

  # Stub methods
  if !respond_to?(:input_name)
    def self.input_name(value, _type = :keyboard)
      return value.to_s
    end
  end

  if !respond_to?(:axis_triggerex?)
    def self.axis_triggerex?(_key)
      return false
    end
  end

  # Stub Controller module for gamepad support
  if !const_defined?(:Controller)
    module Controller
      def self.triggerex?(_key)
        return false
      end
    end
  end
...
#===============================================================================
# BitmapSprite extensions (v22 additions for themed text and image drawing)
#===============================================================================
class BitmapSprite < Sprite
  attr_reader :text_themes

  alias v22_ui_base_initialize initialize
  def initialize(width, height, viewport = nil)
    @text_themes = {}
    v22_ui_base_initialize(width, height, viewport)
  end
...
module UI
  #=============================================================================
  # Mixin for sprite container functionality
  #=============================================================================
  module SpriteContainerMixin
    UI_FOLDER         = "Graphics/UI/"
    GRAPHICS_FOLDER   = ""   # Subfolder in UI_FOLDER
    DEFAULT_TEXT_COLOR_THEMES = {
      :default => [Color.new(88, 88, 80), Color.new(168, 184, 184)],
  ...
  class BaseVisuals
    ...
    # Add QUICK_UP/QUICK_DOWN if they exist in this version
    if defined?(Input::QUICK_UP)
      INPUT_ICONS_ORDER.push(Input::QUICK_UP)
    end
    if defined?(Input::QUICK_DOWN)
      INPUT_ICONS_ORDER.push(Input::QUICK_DOWN)
    end
...
#===============================================================================
# Override PokemonOptionScreen to launch the V22 tabbed Options UI.
# This replaces the v21 flat options list with v22's tabbed pages.
#===============================================================================
class PokemonOptionScreen
  def pbStartScreen(in_load_screen = false)
    UI::Options.new(in_load_screen).main
  end
end
