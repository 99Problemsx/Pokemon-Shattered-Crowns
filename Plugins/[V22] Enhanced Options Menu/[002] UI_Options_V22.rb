...
class UI::OptionsVisuals < UI::BaseVisuals
  ...
  if Input::V22_REMAPPING_AVAILABLE
  ...
  PAGE_HANDLERS.add(:plugins, {
    :name  => proc { next _INTL("Plugins") },
    :order => 50,
    :description => proc { next _INTL("Settings added by plugins.") }
  })
  ...
  def all_pages
    ...
    ret.map! { |val| val[0] }
    # Hide pages that have no options assigned to them
    ret.select! { |page| @options.any? { |opt| opt[:page] == page } }
    return ret
  end
...
class UI::Options < UI::BaseScreen
  ...
  def get_all_options
    ...
    MenuHandlers.each_available(:options_menu) do |option, hash, name|
      # --- Detect v21-format handler and convert to v22 format ---
      handler_type = hash["type"]
      is_v21 = handler_type.is_a?(Class)
      if is_v21
        # Convert v21 type classes to v22 type symbols
        v22_type = case handler_type.name
                   when "EnumOption"   then :array
                   when "SliderOption" then :number_slider
                   when "NumberOption" then :number_type
                   else :array
                   end
        # Infer v22 page from v21 "category" field, or default to :plugins
        v22_page = :plugins
        if hash["category"]
          category_map = {
            :battle   => :gameplay,
          ...
          v22_page = category_map[hash["category"]] || :plugins
        end
        # Convert v21 parameters format for number types
        params = hash["parameters"]
        if v22_type == :number_slider && params.is_a?(Array) && params.length == 3
          params = [params[0], params[1], params[2]]
        end
      else
        v22_type = handler_type
        v22_page = hash["page"]
        params   = hash["parameters"]
      end
      # --- Build the option hash ---
      ...
      ret.last[:parameters].map! { |val| _INTL(val) } if ret.last[:type] == :array && ret.last[:parameters].is_a?(Array)
    end
    return ret
  end
...
end   # if Input::V22_REMAPPING_AVAILABLE
