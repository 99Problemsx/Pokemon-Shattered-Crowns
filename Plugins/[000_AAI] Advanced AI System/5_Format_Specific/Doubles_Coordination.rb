...
class Battle::AI
  def apply_doubles_coordination(score, move, user, target, skill = 100)
    ...
    if target
      ...
      # Gen 9 items
      score += DoublesCoordination::ItemAwarenessDoubles.check_covert_cloak(@battle, target, move, skill)
      score += DoublesCoordination::ItemAwarenessDoubles.check_clear_amulet(@battle, target, move, skill)
      score += DoublesCoordination::ItemAwarenessDoubles.check_mirror_herb(@battle, user, target, move, skill)
      score += DoublesCoordination::ItemAwarenessDoubles.check_ability_shield(@battle, target, move, skill)
    end
    # Loaded Dice + Booster Energy check the attacker's item
    score += DoublesCoordination::ItemAwarenessDoubles.check_loaded_dice(@battle, user, move, skill)
    score += DoublesCoordination::ItemAwarenessDoubles.check_booster_energy(@battle, user, skill)
