...
module AdvancedAI
  module PivotMoves
    ...
    def self.evaluate_parting_shot(battle, attacker, skill_level)
      ...
      # Parting Shot targets ONE opponent — evaluate each, take the best
      best_target_score = opponents.map do |opp|
        ts = 0
        ...
        # Competitive / Defiant punish stat drops with +2 boost — avoid triggering
        if opp.hasActiveAbility?(:COMPETITIVE) || opp.hasActiveAbility?(:DEFIANT)
          ts -= 50
        end
        # Contrary inverts drops into boosts
