...
An AI opponent whose only move was False Swipe just stood there each
turn. Two interacting issues:

1. **`0_Move_Scorer.rb` hard-penalised False Swipe to `-999` in trainer
   battles.** Removed the special-case — False Swipe is a weak 40 BP
   move that scores low enough on its own merits, and the AI still
   uses it when it's the only option.
...
2. **The real bug — `005_AI_ChooseMove.rb#pbChooseMove` registered no
   ...
   With an only-move hard-penalised to -999/-1000 (False Swipe's old
   score, Fake Out after turn 1, an only-move that's Disabled /
   Imprisoned / Choice-locked, or all moves at 0 PP), `max_score` goes
   negative, every choice weight `c[3]` becomes 0, `total_score` is 0,
   `pbAIRandom(0)` returns 0, and the weighted-pick loop skips every
   choice — no move registered, turn passes empty.

   Added a fallback: if the weighted pick registers nothing, force the
   highest-scored move; if that can't register either,
   `pbAutoChooseMove` (Struggle). Catches every hard-penalty-only-move
   case, not just False Swipe.
